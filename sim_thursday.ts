// Simulation: fill Thursday (Franciacorta, 9H) with random gross 3-8 for all 16 players.
// Computes net + Stableford using the same WHS formulas as the app.
...
const rand38 = () => 3 + Math.floor(Math.random() * 6); // 3..8 inclusive
...
async function run() {
  const { data: courseRow } = await supabase
    .from("courses").select("id,holes,par_total,slope,cr,holes_data").eq("id", "franciacorta").single();
  if (!courseRow) throw new Error("Franciacorta not found");
